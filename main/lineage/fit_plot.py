...
for key, fits in lfitr.FITS.items():
    if key in ['8_4', '8']:
        ...
        # All results.
        scores = [lfit.compute_n_plot_gcurves(
            fit, kwarg=par_space, is_plotted=False,
            simulation_count=SIMU_COUNT, is_printed=False) for fit in fits]
        par_sensitivity = [lfit.point_to_cost_fct_parameters(
            point, kwarg=par_space) for point in fits]
        # mfct.plot_laws_s(par_sensitivity, idx_best=np.argmin(scores),
        #                   fig_name=key, fig_subdirectory=FIG_DIR,
        #                   is_zoomed=True)
        ...
        if FORMAT == 'manuscript':
            if key == '10':
                ...
                bbox_to_anchor = None
                FIG_SIZE_PARS = (5.5, 3)
            mfct.plot_laws(parameters, is_par_plot=True, fig_name=key,
                           fig_subdirectory=FIG_DIR, fig_size=FIG_SIZE_PARS)
        # mfct.plot_laws(parameters, is_par_plot=False, fig_name=key,
