...
for key in KEYS:
    ...
    if IS_SAVED:
        ...
        SKEY = None
    cycles = mk.extract_cycles_dataset(out, gal_delay=idx_nogal - idx_gal,
                                       dox_frame_idx=idx_dox, saved_key=SKEY)
    ...
    if IS_PRINTED:
        ...
        print("Number of cycles in the dataset 'raf_dox'",
              len(cycles['raf_dox']))


# Gather both datasets to make the dataset of 'noFC_n2'.
mk.print_if_true('\n' + 'noFc_n2', IS_PRINTED)
out = mk.gather_cycle_dataset(
    ['noFc_1', 'noFc_2'], 'noFc_n2', is_printed=IS_PRINTED)
...
NCYCLES, LCYCLES = mk.make_cycles_dataset('noFc_n2', thresholds=THRESHOLDS,
...
for key in list(mk.IDXS_CDT.keys()):
    if key != 'noFc_noCas9_noDox':
        ...
        DATA_EXP[key] = mk.postreat_experimental_lineages_from_path(
            path, 'results_analysis',  threshold=THRESHOLDS['raf'],
            gen_count_min=1, par_multiple_thresholds=[THRESHOLDS['gal'],
                                                      mk.IDXS_CDT[key][2]])
        # # Save.
...
for key in ['noFc_n2', 'Fc0_n2', 'Fc20_n2', 'Fc30_n2', 'Fc40_n2', 'Fc50_n2',
    ...
    DATA_EXP[key] = mk.gather_postreated_output(DATA_EXP[key_1],
                                                DATA_EXP[key_2])
...
if IS_PRINTED:
    ...
    print(f"Proportion of long cycle in {data_key} under raf: "
          f"{out['raf_dox']} %")
