...
from telomeres.dataset.extract_processed_dataset import extract_distribution_cycles
from telomeres.auxiliary.figures_properties import (
    LABELS as LABELS_FP,
    MY_PALETTE,
    MY_COLORS,
)
from telomeres.model.posttreat import transform_l_init
from telomeres.auxiliary.write_paths import characteristics_to_string, FOLDER_FIG
...
def plot_data_exp_concentration_curves(
    ...
    plt.errorbar(
        x, stat_telo_p["avg"], yerr=stat_telo_p["std"], capsize=2, label=LABELS["telo+"]
    )
    plt.errorbar(
        x, stat_telo_m["avg"], yerr=stat_telo_m["std"], capsize=2, label=LABELS["telo-"]
    )
...
def plot_ltelomere_init_wrt_par(
    ltrans=0,
    l0=0,
    l1=0,
    distribution=None,
    ...
    for i in range(par_count):
        par_l_init = [ltrans_s[min(i, a - 1)], l0_s[min(i, b - 1)], l1_s[min(i, c - 1)]]
...
def plot_n_print_l_init(distribution, fig_subdirectory, fig_name=None, color="grey"):
...
def plot_cycles_from_dataset(fig_subdirectory, is_saved=False):
    ...
    fig, axes = plt.subplots(1, 1)
    sns.kdeplot(
        data=[cycles[key] for key in keys][::-1],
        log_scale=False,
        fill=True,
        common_norm=False,
        palette=MY_PALETTE,
