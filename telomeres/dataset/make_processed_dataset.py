...
def make_distributions_cycles(data, threshold, cdt_min, sfolder=None):
    ...
    is_dead_s = is_as_expected_lineages(
        gtrigs, lineage_types, is_accidental_deaths, ["dead"]
    )
    is_atype_s = is_as_expected_lineages(
        gtrigs, lineage_types, is_accidental_deaths, ["atype"]
    )
    is_btype_s = is_as_expected_lineages(
    ...
    if sfolder is not None:
        ...
        cdts_final = {
            k: v for k, v in cdts_.items() if k in ["norA", "norB", "nta", "sen"]
        }
...
def make_microfluidic_dataset(
    file_name, parameters=PAR_CYCLES_POSTREAT, is_saved=True, strain=""
):
    ...
    df = pd.DataFrame(data[0]["cycle"][::-1], index=np.arange(len(data[0]["cycle"])))
...
def postreat_population_concentration():
    ...
    data = {}  # Data.
    data["DOX+"] = np.array(
        pd.read_excel(path_raw, header=None, sheet_name="raw-DOX+_anais")
    )
    data["DOX-"] = np.array(
        pd.read_excel(path_raw, header=None, sheet_name="raw-DOX-_anais")
    )
    ...
    if os.path.exists(path_pol32):
        ...
        keys_replicate = {  # All the replicates of the strains of interest.
            "tlc1": ["MATa tlc1-1c", "MATa tlc1-2c", "MATa tlc1-3b", "MATa tlc1-4c"],
            "tlc1_pol32": [
                "MATa tlc1 pol32-3d",
                "MATa tlc1 pol32-4b",
                "MATa tlc1 pol32-5a",
    ...
    for key_strain, d in data_t.items():
        stats[key_strain] = {
            k: {"avg": np.mean(d[k], axis=1), "std": np.std(d[k], axis=1), "all": d[k]}
            for k in d
        }
    return stats
...
def extract_population_lmode_from_raw():
    path = os.path.join(DIR_DATA_RAW, "BioRad_2012-04-26_18hr_36min_analysis.txt")
...
def make_population_dataset(is_saved=True):
    ...
    if is_saved:
        for key, stat in stats.items():
            ...
            if not os.path.exists(folder):
                os.makedirs(folder)
            np.save(os.path.join(folder, f"cell_concentration_{key}.npy"), stat)
            # Save in csv for the `source data` file (not used by python).
            if key == "DOX-":
                data_article = stat["OD"]
                df = pd.DataFrame.from_dict(data_article, orient="index").T
                df.to_csv(os.path.join(folder, "Source Data SFig3a.csv"), index=False)
            if key == "DOX+":
                data_article = stat["OD"]
                df = pd.DataFrame.from_dict(data_article, orient="index").T
                df.to_csv(
                    os.path.join(folder, "Source Data Fig3b-exp.csv"), index=False
                )
            if key == "tlc1_pol32":
                data_article = stat["c"]
                df = pd.DataFrame.from_dict(data_article, orient="index").T
                df.to_csv(os.path.join(folder, "Source Data SFig3c-.csv"), index=False)
            if key == "tlc1":
                data_article = stat["c"]
                df = pd.DataFrame.from_dict(data_article, orient="index").T
                df.to_csv(os.path.join(folder, "Source Data SFig3c+.csv"), index=False)
            if key == "RAD51":
                data_article = stat["OD"]
                df = pd.DataFrame.from_dict(data_article, orient="index").T
                df.to_csv(os.path.join(folder, "Source Data SFig6d.csv"), index=False)
