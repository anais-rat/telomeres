...
def population_init(c_init_s, par_l_init, rng):
    ...
    ccum_s = np.zeros(subpop_count + 1, dtype=int)
    ccum_s[1:] = np.cumsum(c_init_s)
...
def population_evolution(
    ...
    if cell_count_init == 0:
        raise Exception(
            "Function `population_evolution` does not accept empty populations."
        )
    ...
    for i in range(1, time_count):
        ...
        for cell in div_idxs:
            ...
            if (sat_choice == "prop" and c_current >= c_sat) or (
                ...
                d["evo_lmode"][time_saved_idx:] = d["evo_lmode"][time_saved_idx - 1]
                # Or add empty list for remaining saved times
                # (except `lmins_gsen` for current time).
                time_left_count = time_count_saved - len(evo_lmin_gsen["atype"]) - 1
        ...
        if dead_count > 0:
            ...
            if c_current == 0:
                ...
                extinction_time = np.max(np.array(history_dead[-dead_count:])[:, 2]) * (
                    60 * 24
                )
        ...
        if i == time_saved_idxs[time_saved_idx]:
            ...
            d["evo_lmode"][time_saved_idx] = np.transpose(
                stats.mode(
                    d["lengths"][:c_idx_max].flatten().astype("float"),
                    nan_policy="omit",
                )
            )
            time_saved_idx += 1
...
def gather_evo_and_dilute(output_s, c_dilution, para_count, gen_count_previous, rng):
    ...
    with warnings.catch_warnings():
        ...
        out["evo_lmin_max"] = np.nanmax(
            [output_s[i]["evo_lmin_max"] for i in subpops], axis=0
        )
        out["evo_lmin_min"] = np.nanmin(
            [output_s[i]["evo_lmin_min"] for i in subpops], axis=0
        )
        ...
        out["evo_lmode"] = afct.nansum(
            [output_s[i]["evo_lmode"][:, 0] for i in subpops] * weights, axis=0
        )

    # Computation of population's history data:
    # > Concatenation of subpopulations' data.
    out["history_dead"] = np.concatenate(
        [output_s[i]["history_dead"] for i in subpops], axis=0
    )
    time_count = len(output_s[0]["evo_lmin_gsen"]["atype"])
    out["evo_lmin_gsen"] = {
        key: [
            list(
                np.concatenate([output_s[i]["evo_lmin_gsen"][key][j] for i in subpops])
            )
    ...
    for s in range(subpop_count):
        ...
        for key in ks.data_keys:
            data_s[s][key] = np.array(
                [output_s[idxs[i, 0]][key][idxs[i, 1]] for i in rg]
            )
...
def simu_parallel(
    ...
    for key in ks.data_saved_keys:
        d["day_init_data"][key] = [
            np.concatenate([data_s[s][key] for s in range(simu_count)])
        ]
    ...
    while (is_to_extinction and (not np.isnan(c_current))) or (
        ...
        if day == 0:
            ...
            d["history_dead"] = np.append(
                d["history_dead"], temp["history_dead"], axis=0
            )
        ...
        if np.isnan(c_current):
            ...
            if is_to_extinction:
                time_idxs = np.concatenate([tsaved_idxs[d] for d in range(day + 1)])
        ...
        if is_to_extinction and (day + 1 >= day_count):
            ...
            tsaved_idxs[day + 1] = np.arange(
                first_idx,
                first_idx + psim["t_day_count"] + 1,
                psim["t_day_count"] // psim["t_day_count"],
            )
            tsaved_idxs[day + 1][-1] -= 1

            times = np.append(
                times[:-1], times[: dil_idxs[0] + 1] + day * times[dil_idxs[0]]
            )
        day += 1
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        d["extinction_time"] = np.nanmax(
            [output_s[sim]["extinction_time"] for sim in range(simu_count)]
        ) / (60 * 24)
