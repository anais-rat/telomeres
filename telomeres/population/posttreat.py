...
from telomeres.dataset.extract_processed_dataset import extract_population_lmode
...
def make_time_arrays(par_sim, is_printed=True):
    ...
    if r != 0 and is_printed:
        print(
            "WARNING: the actual number of times saved per day is not "
...
def make_hist_lmin_gsen_from_evo(evo_lmin_gsen, x_axis=par.X_AXIS_HIST_LMIN):
    ...
    for key in ks.type_keys:
        ...
        hist["hist_lmin_all"][key] = fct.make_histogram(lmins_gsen, x_axis, False)
        hist["hist_lmin_per_day"][key] = [
            fct.make_histogram(lmins, x_axis, False) for lmins in lmins_gsen_per_day
        ]
    return hist
...
def postreat_from_evo_c_from_output(output, file_postreated_name):
    ...
    for key in ks.evo_c_anc_keys:
        ...
        if sum_key not in ["evo_c", "evo_c_sen"]:  # If not already computed.
...
def postreat_performances(
    ...
    out_processed_path = wp.write_sim_pop_postreat_perf(sub_dir_path, simu_count)
...
def statistics_simus_from_path(folder, simu_count):
    ...
    sat_times_s = np.array(
        [fct.reshape_with_nan(sat_times_s[i], max(2, sat_day_count)) for i in simus]
    )
    ...
    anc_s = {
        "by_lmin": [np.argsort(lmin_init_s[i]) for i in simus],
    ...
    es["hist_lmin_per_day"] = {
        key: [statistics([hist[key][day] for hist in hist_s]) for day in days]
        for key in ks.type_keys
    }
...
def statistics_simus_from_path_if_not_saved(folder_name, simu_count, is_loaded=True):
...
def statistics_simus(
    ...
    return statistics_simus_from_path_if_not_saved(
        sub_dir_path, simu_count, is_loaded=is_loaded
    )
...
def statistics_simus_csv(para_count, cell_count, simu_count, par_update=None):
    ...
    """
    folder = wp.write_simu_pop_subdirectory(
        cell_count, para_count, par_update=par_update
    )
    simus = np.arange(simu_count)
    # Load paths to all simulations in a list.
