...
def write_exp_law(parameters, law_name):
    a, b = parameters[:2]
    string = rf"$a_{{ {law_name} }} = {a}, ~$" + rf"$b_{{ {law_name} }} = {b}, ~$"
...
def write_laws_s(parameters, decimal_count=DECIMAL_COUNT):
    ...
    senA_formatted = list_to_strings(
        par_sen[0], is_last_int=True, decimal_count=decimal_count
    )
    if par_sen[0] != par_sen[1]:
        laws["psenA"] = write_exp_law(senA_formatted, "senA")
        senB_formatted = list_to_strings(
            par_sen[1], is_last_int=True, decimal_count=decimal_count
        )
...
def print_data_on_special_initial_distributions(l_short, l_medium, l_long):
    ...
    print(
        f"Average length |  {np.mean(l_long)} |   "
        f"{np.mean(l_medium)}   |   {np.mean(l_short)}     |"
    )
    print(
        f"Minimum length |    {np.min(l_long)}   |"
        f"     {np.min(l_medium)}    |"
        f"     {np.min(l_short)}    |"
    )
    print(
        f"Maximum length |    {np.max(l_long)}   |"
        f"     {np.max(l_medium)}    |"
        f"    {np.max(l_short)}    |"
    )
    return None
...
def plot_laws_nta_various_a(
    a_to_test,
    b,
    lengths=LENGTHS,
    law_nta=law_nta_usual,
    ...
    for a in a_to_test:
        plt.plot(
            lengths, law_nta(lengths, a, b), label=rf"$a_{{nta}} = {a}$", linewidth=2.5
        )
...
def plot_laws_nta_various_b(
    ...
    for b in b_to_test:
        plt.plot(
            lengths, law_nta(lengths, a, b), label=rf"$b_{{nta}} = {b}$", linewidth=2.5
        )
        plt.xlabel(LABELS["ax_lmin_min"], labelpad=6)
...
def plot_laws_s(
    ...
    for fig_size in fig_sizes:
        fig_all, axes_all = plt.subplots(
            1, len(probas), sharex=True, sharey=True, figsize=fig_size
        )
        ...
        for key, proba in probas.items():
            ...
            axes_all[i].text(
                ...
                ha="right",
                va="top",
                transform=axes_all[i].transAxes,
                bbox=dict(boxstyle="round", fc="w", ec="w"),
            )
            axes[i].plot(
                lengths, proba["mean"], label=LABELS["avg"], color="darkorange"
            )
            axes[i].fill_between(
                lengths,
                *proba["ext"],
...
def plot_laws(
    ...
    plt.plot(
        lengths, law_nta(lengths, *parameters[0]), label=labels["pnta"], color=COLORS[0]
    )
    if len(parameters[1]) == 3:
        plt.plot(
            lengths,
            law_sen(lengths, *parameters[1]),
            label=labels["psen"],
            color=COLORS[0],
        )
        ...
        plt.plot(
            lengths,
            law_sen(lengths, *parameters[1][0]),
            label=labels["psen"],
            color=COLORS[1],
        )
    else:
        ...
        plt.plot(
            lengths,
            law_sen(lengths, *parameters[1][1]),
            label=labels["psenB"],
            color=COLORS[2],
        )
    plt.ylim(-0.1, 1.1)
    ...
    if is_par_plot:
        plt.legend(
            title=labels["linit"],  # loc="upper right")
            bbox_to_anchor=(1, 1),
            loc="lower right",
        )
    else:
...
def plot_distributions_shortest_wrt_linit(
    par_l_inits, cell_count, is_rescaled=False, fig_supdirectory=None
):
    ...
    for i in range(curve_count):
        lengths, distrib = distribution_shortest(cell_count, par_l_init=par_l_inits[i])
        if is_rescaled:
            distrib = distrib / np.max(distrib)
        if cell_count == 1e5:
            plt.plot(lengths[:350], distrib[:350], label=r"$N_{exp}$", color=colors[i])
        else:
            plt.plot(
                lengths[:350],
                distrib[:350],
                label=rf"${par_l_inits[i]}$",
                color=colors[i],
            )
    ...
    plt.ylabel("Density", labelpad=6)
    for i in range(curve_count):
        lengths, distrib = distribution_shortest_max(
            cell_count, par_l_init=par_l_inits[i]
        )
        if is_rescaled:
            distrib = distrib / np.max(distrib)
        if cell_count == 1e5:
...
def plot_distributions_shortest_min(
    cell_counts, is_rescaled=False, fig_subdirectory=None
):
    ...
    if not isinstance(fig_subdirectory, type(None)):
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        fig_name = (
            "distributions_shortest_min_c" + wp.list_to_string(cell_counts) + ".pdf"
        )
...
def plot_distributions_shortest(cell_counts, is_rescaled=False, fig_subdirectory=None):
    cell_counts = np.sort(cell_counts).astype(int)
    curve_count = len(cell_counts)
    ...
    for i in range(curve_count):
        ...
        if cell_counts[i] == 1e5:
            ...
            axes[0].plot(distrib_min[:350], label=cell_counts[i], color=colors[i])
            axes[1].plot(distrib_max[:350], color=colors[i])
