...
data_saved_keys = ["nta_counts", "lengths", "ancestors", "generations", "sen_counts"]
...
evo_c_anc_keys = ["evo_c_ancs", "evo_c_B_ancs", "evo_c_B_sen_ancs", "evo_c_sen_ancs"]
...
evo_p_anc_keys = [
    evo_p_anc_keys[i].replace("_c", "_p") for i in range(len(evo_c_anc_keys))
]
if HTYPE_CHOICE:
    evo_p_anc_keys.append("evo_p_H_sen_ancs")

evo_c_keys_new = [
    evo_c_anc_keys[i].replace("_ancs", "") for i in range(len(evo_c_anc_keys))
]
