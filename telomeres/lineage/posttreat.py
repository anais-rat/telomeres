...
def is_as_expected_lineage(gtrigs, lineage_type, is_accidental_death, characteristics):
...
def is_as_expected_lineages(
    gtrigs_s, lineage_types, is_accidental_deaths, characteristics
):
    ...
    if "htype" in characteristics:
        is_as_expected = np.logical_and(is_as_expected, np.isnan(lineage_types))
    is_w_arrested = np.array(["arrested" in c for c in characteristics])
    if any(is_w_arrested):
        arrested_charac = [
            characteristics[i] for i in np.arange(len(is_w_arrested))[is_w_arrested]
        ]
...
def select_exp_lineages(exp_data, characteristics):
    ...
    selected_lineage_idxs = is_as_expected_lineages(
        gtrigs, lineage_types, is_accidental_deaths, characteristics
    )
    ...
    for key in lcycle_per_seq_counts:
        lcycle_per_seq_counts[key] = lcycle_per_seq_counts[key][selected_lineage_idxs]
    selected_data = (
        evo,
        gtrigs,
        lineage_types,
...
def sort_lineages(data, type_of_sort):
    ...
    if type_of_sort == "gdeath":
        ...
        raise ValueError(
            "ERROR: wrong `type_of_sort` argument for `sort_lineage` function"
        )
    # Ordering of data.
    # > `evo_s`.
    ...
    for lin in range(lineage_count):
        ...
        if nta_max > 0:
            temp.append(
                np.nansum(
                    is_lcycle[
                        lin, int(gtrigs["nta"][lin, nta_max - 1]) : idx_nta_max_end
                    ]
                )
            )
        ...
        if not np.isnan(gtrigs["sen"][lin]) or np.isnan(gtrigs["death"][lin]):
            lcycle_per_seq_counts["sen"][lin] = (
                gtrigs["death"][lin] - gtrigs["sen"][lin] + 1
            )
    return lcycle_per_seq_counts


def postreat_experimental_lineages(
    data, threshold, gen_count_min, par_multiple_thresholds=None
):
    ...
    for i in lineages:
        ...
        if isinstance(par_multiple_thresholds, type(None)):  # Only 1 threshold
            ...
            is_long.append(
                np.append(
                    cycles[i][:idx_change] > threshold_new,
    ...
    cycles = np.array([reshape_with_nan(cycles[i], gen_count) for i in lineages_kept])
    is_long = np.array([reshape_with_nan(is_long[i], gen_count) for i in lineages_kept])
    ...
    is_long_with_1st_normal = np.append(
        np.zeros((lineage_kept_count, 1)), is_long, axis=1
    )
    gtrigs_temp = [
        np.where(is_long_with_1st_normal[i, 1:] - is_long_with_1st_normal[i, :-1] == 1)[
            0
        ]
        for i in range(lineage_kept_count)
    ]
    ...
    for lin_idx in range(lineage_kept_count):
        ...
        if nta_counts[lin_idx] > 0:
            ...
            if is_last_long[lin_idx]:  # last arrest counted as sen entry.
                ...
                if nta_counts[lin_idx] > 0:
                    gtrigs["nta"][: nta_counts[lin_idx], lin_idx] = gtrigs_temp[
                        lin_idx
                    ][:-1]
    ...
    is_type_a = np.logical_and(
        is_dead, np.logical_and(nta_counts == 0, ~np.isnan(gtrigs["sen"]))
    )
