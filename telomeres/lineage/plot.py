...
LABEL_HIST_X = {'nta': 'Number of consecutive arrests',
                'nta_total': r'Number ' + msg}
LABEL_HIST_Y = {'nta_total': 'Percent of (non-terminally arrested) lineages',
...
PAR_RC_HEATMAP = {'axes.facecolor': ".94",
                  'legend.frameon': True,
                  'legend.framealpha': 1,
...
def compute_n_plot_gcurve_error(exp_data_raw, lineage_count_on_all_simu,
    ...
    sim_data_s = sim.simulate_n_average_lineages(
        lineage_count, simulation_count, gcurves, characteristics,
    ...
    for gcurve in gcurves:
        ...
        if isinstance(sim_data_s[gcurve], type(None)):
            ...
            if is_plotted:
                ...
                ax = plt.gca()
                plt.text(.01, .97, write_laws(p_update['fit']),
...
def compute_n_plot_gcurve(exp_data, simu_count, characteristics,
                          fig_subdirectory, gcurve=None, type_of_sort=None,
                          par_update=None, is_exp_plotted=False,
                          bbox_to_anchor=None, title=None, is_propB=False,
                          proc_count=1, add_to_name=None, tick_spacing=None):
...
def compute_n_plot_gcurves_wrt_charac(exp_data, simu_count, characteristics_s,
    ...
    for i in range(fig_count):
        ...
        lineages, gtrigs_exp, gtrigs_sim = sim.compute_gtrigs(
            exp_data, simu_count, characteristics, gcurve, gcurve,
            par_update=p_update, proc_count=proc_count)
    ...
    sns.despine()
    if not isinstance(fig_subdirectory, type(None)):
        path = path or wp.write_gcurves_path(par_update=p_update,
                                             subdirectory=fig_subdirectory)
        if not isinstance(add_to_name, type(None)):
            path = path.replace('.pdf', f'_{add_to_name}.pdf')
        plt.savefig(path, bbox_inches='tight')
...
def compute_n_plot_gcurves_wrt_sort_n_gen(exp_data, simulation_count,
    ...
    for i in range(len(gcurves)):
        ...
        lineages, gtrigs_exp, gtrigs_sim = sim.compute_gtrigs(
            exp_data, simulation_count, characteristics, gcurve, type_of_sort,
            par_update=p_update)
        LINESTYLE = '-'
        if is_exp_plotted:
...
def plot_gcurves_wrt_par(exp_data, simu_count, characteristics,
                         varying_par_updates, varying_key, fig_subdirectory,
    ...
    for i in range(par_count):
        ...
        lineages, gtrigs_exp, gtrigs_sim = sim.compute_gtrigs(
            exp_data, simu_count, characteristics, gcurve, type_of_sort,
            par_update=p_update)
...
def plot_medians_wrt_par(exp_data, simu_count, characteristics, varying_pars,
    ...
    for i in range(par_count):
        p_update = {'is_htype_seen': False}  # Default for comparison w exp
        p_update.update(varying_par_updates[i] or {})
        lineages, gtrigs_exp, gtrigs_sim = \
            sim.compute_gtrigs(exp_data, simu_count, characteristics, gcurve,
                               type_of_sort, par_update=p_update)
    ...
    plt.hlines(y=y_exp, xmin=xmin, xmax=x_exp, color='darkorange', ls='--',
               label=LABELS['exp'])
...
def plot_histogram(x_axis, y_axis, width=1, normalized=True, ylim=None,
                   fig_subdirectory=None, title=None):
    plt.figure()
...
def compute_n_plot_hist_gen_coupure(gsen_exp, fig_subdirectory,
                                    x_axis=np.arange(7), width=1,
                                    normalized=True, ylim=None, title=None):
    y_axis = fct.make_histogram_wo_nan(gsen_exp, x_axis=x_axis,
                                       normalized=normalized)
    plot_histogram(x_axis, y_axis, width=width, normalized=normalized,
                   fig_subdirectory=fig_subdirectory, ylim=ylim, title=title)
    return y_axis


def compute_n_plot_hist_lmin(exp_data, simulation_count, characteristics_s,
                             hist_lmins_axis, fig_subdirectory,
    ...
    lineage_counts, data_s = sim.compute_lmin_histogram_data(
        exp_data, simulation_count, characteristics_s, par_update=p_update,
        hist_lmins_axis=hist_lmins_axis,
        lineage_count_on_all_simu=lineage_count_on_all_simu)
    hist_s = []
    for i in range(char_count):
        axis, hist = data_s[i]
        axis_new, hist_new = fct.rescale_histogram_bin(axis, hist['mean'],
                                                       width)
        if i == 0:
        ...
        if not isinstance(fig_subdirectory, type(None)):
            ...
            path = wp.write_hist_lmin_path(simulation_count, lineage_counts[i],
    ...
    if char_count > 1:
        ...
        if (['btype', 'senescent'] in characteristics_s
            ...
            hist_s = [np.append(hist, np.zeros(x_max - len(hist))) for hist in
                      hist_s]
            ...
            plt.bar(axis[:x_count], weight * hist_s[idx_a] + (1-weight) *
                    hist_s[idx_b], bottom=(1-weight) * hist_s[idx_b],
                    width=width, color=fp.COLORS_TYPE['atype'],
                    label=LABELS['atype'])
            handles, labels = plt.gca().get_legend_handles_labels()
            plt.legend(handles[::-1], labels[::-1], title='Cell type',
                       loc="upper right")
...
def compute_n_plot_lcycle_hist(exp_data, simulation_count, characteristics,
    ...
    (lineage_count, lcycle_counts_exp, lcycle_counts_sim,
     lcycle_counts_sim_h) = sim.compute_lcycle_histogram_data(
    ...
    if is_saved:
        ...
        path_exp = wp.write_hist_lc_exp_path(lineage_count, characteristics,
                                             subdirectory=fig_subdirectory)
        path_sim = wp.write_hist_lc_path(simulation_count, *kwarg,
                                         par_update=p_update,
                                         subdirectory=fig_subdirectory)
        if par.HTYPE_CHOICE:
            p_update['is_htype_seen'] = True
            path_sim_h = wp.write_hist_lc_path(simulation_count, *kwarg,
                                               par_update=p_update,
                                               subdirectory=fig_subdirectory)
    else:
        path_exp = None
    ...
    for key in ['gen', 'time']:
        ...
        for i in range(3):
            ...
            if data_key == 'lmin_min':
                ...
                plt.plot(x_ax, y_axs['mean'], label=LABELS[data_key + '_avg'],
                         color=COLORS_R[i])
